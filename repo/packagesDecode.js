...
function decodePackagesFile(packagesFile) {
    ...
    for (packageNum = 0; packageNum < packages.length; packageNum++) {
        ...
        for (i=0; i<properties.length; i++) {
            ...
            try {
                ...
                console.log(packages[packageNum]);
    ...
    // Split the data into individual package blocks
    const packageBlocks = packagesFile.trim().split('\n\n');
    ...
    packageBlocks.forEach(block => {
        ...
        lines.forEach(line => {
            ...
            if (key === 'Description') {
                // If Description already exists, concatenate with newline
                packageObj[key] = packageObj[key] ? packageObj[key] + '\n' + value : value;
            } else {
