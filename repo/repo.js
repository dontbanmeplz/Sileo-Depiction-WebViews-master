...
async function getRepo(repoURL) {
    ...
    repoTitle = repoURL.split(".").reverse()[1].replace("https://","");
    // Set Repo Title
    ...
    try {
        ...
        var packagesFile = await corsBypass(repoURL + "Packages.bz2");
        console.log(1);
            ...
            for (j=0; j<packages.length; j++) {
                ...
                if (packages[j].Section == categories[i].name) {
                    ...
                    if (packages[j].hasOwnProperty("SileoDepiction")) {
                        ...
                        if (packages[j].hasOwnProperty("Filename")) {
                            var ur = packages[j].SileoDepiction.split("/")[2];
                            console.log("https://" + ur + packages[j].Filename);
                            tableCellLink.href = "https://" + ur + "/" + packages[j].Filename;
                        }
                    } 
